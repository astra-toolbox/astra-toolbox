...
./autogen.sh
for PYTHON_VERSION in 3.10 3.11 3.12 3.13 3.14 3.14t ; do
    ./configure --with-python=python${PYTHON_VERSION} \
                --with-cuda=/usr/local/cuda-12.9 \
    ...
    auditwheel repair --plat manylinux_2_28_x86_64 --exclude "libcudart.so.*" --exclude "libcufft.so.*" python/dist/*.whl
