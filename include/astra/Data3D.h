...
public:
	...
	CDataMemory(size_t size) { _allocateData(size); }
...
protected:
	G* m_pGeometry;
	CData3DObject(int x, int y, int z, G *geom, CDataStorage *storage) : CData3D(x, y, z, storage), m_pGeometry(geom->clone()) { }
...
class CFloat32ProjectionData3D : public CData3DObject<CProjectionGeometry3D> {
public:

	CFloat32ProjectionData3D(CProjectionGeometry3D *geom, CDataStorage *storage) : CData3DObject<CProjectionGeometry3D>(geom->getDetectorColCount(), geom->getProjectionCount(), geom->getDetectorRowCount(), geom, storage) { }
...
class CFloat32VolumeData3D : public CData3DObject<CVolumeGeometry3D> {
public:
	CFloat32VolumeData3D(CVolumeGeometry3D *geom, CDataStorage *storage) : CData3DObject<CVolumeGeometry3D>(geom->getGridColCount(), geom->getGridRowCount(), geom->getGridSliceCount(), geom, storage) { }
