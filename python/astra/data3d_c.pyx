...
def create(datatype,geometry,data=None, link=False):
    ...
    data_size = geom_shape[0]
    data_size = data_size * geom_shape[1]
    data_size = data_size * geom_shape[2]
    ...
    if datatype == '-vol':
        ...
        if link:
            ...
            pStorage = new CDataMemory[float32](data_size)
        ...
        if link:
            ...
            pStorage = new CDataMemory[float32](data_size)
            pDataObject3D = new CFloat32ProjectionData3D(ppGeometry, pStorage)
        del ppGeometry
    else:
...
cdef fillDataObjectArray(CData3D* obj, float [:,:,::1] data):
    cdef float [:,:,::1] cView = <float[:data.shape[0],:data.shape[1],:data.shape[2]]> obj.getFloat32Memory()
    cView[:] = data
